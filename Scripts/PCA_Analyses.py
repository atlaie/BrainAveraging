...
for m in range(39):
    ...
    dist_remov = np.nan * np.ones((choicess.shape[0], len(dataa.keys())))
    dist_wrong_pop = np.nan * np.ones((choicess.shape[0], len(dataa.keys())))
    dist_wrong_remov = np.nan * np.ones((choicess.shape[0], len(dataa.keys())))
        ...
        if np.unique(b_all[m])[bb] != 'root':
            ...
            if popVectors.shape[1]>1:
                ...
                for i in range(popVectors.shape[0]):
                        if stimDiffs[i] < 0:
                            # dist_pop[i, bb] = nrm(svdd.components_ - a_left)/d_ref
                            # dist_wrong_pop[i, bb] = nrm(svdd.components_ - a_right)/d_ref
                                ...
                                for k in range(numSamp):
                                    ...
                                    if popVectors.shape[1] > 20:
                                        ...
                                        for s in range(numSubRep):
                                            subsam = np.random.choice(popVectors.shape[1],int(np.floor(popVectors.shape[1]/(k+1))))
                                            ...
                                            if nComps > 1:
                                                ...
                                                dist_subsam[i, bb, s, k] = nrm(np.dot(svdd4.components_, singletrial[subsam]) - a_leftt) / d_ref
                                                dist_wrong_subsam[i, bb, s, k] = nrm(np.dot(svdd4.components_, singletrial[subsam]) - a_rightt) / d_ref
                            ...
                            # Shuffling loop.
                                for s in range(numShuf):
                                    ...
                                    dist_Sh[i, bb, s] = \
                                    nrm(ww - a_left)/d_ref
                                    aaa = choices(np.arange(0, a_right.shape[0],1), a_right/np.sum(a_right),
                                                 k=int(np.sum(popVectors[i, :])))

                                    rands = Counter(aaa)
                                    ww = np.zeros_like(a_right)
                                    ww[list(rands.keys())] = list(rands.values())
                                    dist_wrong_Sh[i, bb, s] = \
                                    nrm(ww - a_right)/d_ref

                                perc[i,bb] = sts.percentileofscore(dist_Sh[i, bb, :], dist_pop[i, bb]) / 100
                                perc_wrong[i,bb] = sts.percentileofscore(dist_wrong_Sh[i, bb, :], dist_wrong_pop[i, bb]) / 100
                                        ...
                                        for s in range(numSubRep):
                                            ...
                                            if np.sum(popVectors[:,subsam]) != 0:
                                                ...
                                                if nComps > 1:
                                                    ...
                                                    knee = kn.KneeLocator(np.arange(0, nComps), svdd5.explained_variance_ratio_,
                                                                          curve='convex', direction='decreasing', interp_method='interp1d')
                                                    if knee.knee is not None:
                                                        if knee.knee > 0:
                                                            nComps2 = knee.knee[knee.knee > 0][0]
                                                    ...
                                                    b_left = popVectors[stimDiffs < 0, :]
                                                    b_right = popVectors[stimDiffs > 0, :]
                                                    if np.logical_and(np.sum(np.nanmean(b_left[:,subsam], axis = 0)) > 0,  np.sum(np.nanmean(b_right[:,subsam], axis = 0)) > 0):
                                                        a_leftt = np.dot(svdd6.components_, np.nanmean(b_left[:,subsam], axis = 0))
                                                        a_rightt = np.dot(svdd6.components_, np.nanmean(b_right[:,subsam], axis = 0))
                                                        d_ref = nrm(a_leftt - a_rightt)

                                                        singletrial = popVectors[i, :]

                                                        dist_subsam[i, bb, s, k] = nrm(np.dot(svdd6.components_, singletrial[subsam]) - a_rightt) / d_ref
                                                        dist_wrong_subsam[i, bb, s, k] = nrm(np.dot(svdd6.components_, singletrial[subsam]) - a_leftt) / d_ref
                                                    else:
                            ...
                            if doShuff:
                                #Shuffling loop.
                                for s in range(numShuf):
                                    aa = choices(np.arange(0, a_right.shape[0], 1),
                                                  a_right / np.sum(a_right),
                                                  k=int(np.sum(popVectors[i, :])))
                                    rands = Counter(aa)
                                    ww = np.zeros_like(a_right)
                                    ww[list(rands.keys())] = list(rands.values())
                                    ...
                                    aaa = choices(np.arange(0, a_left.shape[0]), a_left/np.sum(a_left),
                                                 k=int(np.sum(popVectors[i, :])))
                                    rands = Counter(aaa)
                                    ww = np.zeros_like(a_left)
                                    ww[list(rands.keys())] = list(rands.values())
    ...
    if doShuff:
        surrogate_corr.append(dist_Sh)
        surrogate_wrong_corr.append(dist_wrong_Sh)
...
if doSubSam:
    ...
    v_list = [measured_subsam_fin2, measured_subsam_wrong_fin2, dist_hit_subsam_fin2, dist_miss_subsam_fin2, specIdx_hit_subsam_fin2, specIdx_miss_subsam_fin2]
